...
const AvatarContainer = dynamic(
  () => import("@/components/avatar/AvatarContainer"),
  { ssr: false },
);
...
function MessageBubble({ message }: { message: Message }) {
  ...
  return (
    <div
      className={`flex items-end gap-2 message-enter ${isUser ? "flex-row-reverse" : "flex-row"}`}
    >
      <div
      ...
      >
        ...
        <p
          className={`text-[10px] mt-1.5 ${isUser ? "text-white/60 text-right" : "text-white/30"}`}
        >
...
export default function ChatPage() {
  ...
  const [model, setModel] = useState(
    process.env.NEXT_PUBLIC_DEFAULT_MODEL || "llama3",
  );
    ...
    <div className="grain fixed inset-0 flex bg-ink overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          {error && (
            <div className="flex items-start gap-2 animate-fade-in">
              <div className="px-4 py-3 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-300 text-sm">
                ...
                <p className="text-red-300/70 text-xs whitespace-pre-line">
                  {error}
                </p>
              </div>
            </div>
        ...
        <div className="flex-shrink-0 px-6 py-4 border-t border-white/6 bg-white/[0.02]">
          <div
            className={`flex items-end gap-3 bg-white/6 border rounded-2xl px-4 py-3 transition-all duration-200 ${
              isLoading
                ? "border-accent/30"
          ...
          <p className="text-[11px] text-white/20 mt-2 text-center">
            Shift+Enter で改行 · Ollama が localhost:11434
            で起動している必要があります
          </p>
        </div>
      </div>
